  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      for (let j = 0; j < $.shareCode.length && $.canHelp; j++) {
        console.log(`${$.UserName} 去助力 ${$.shareCode[j]}`)
        if ($.UserName == $.shareCode[j].use) {
          console.log(`不能助力自己\n`)
          continue
        }
        $.delcode = false
        await harmony_collectScore({"appId":appId,"taskToken":$.shareCode[j],"actionType":"0","taskId":"6"})
        await $.wait(2000)
        if ($.delcode) {
                ...
                if (vo.status !== 2) {
                  if (vo.taskType === 13) {
                    ...
                    if (vo.times !== vo.maxTimes) {
                      $.shareCode.push(vo.assistTaskDetailVo.taskToken)
                    }
                  }
                } else {
